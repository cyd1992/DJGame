...
//	}
	...
	while (noteElement)
	{
		...
		while (element)
		{
			...
			else if (strcmp(element->Value(), "track") == 0)
			{
				note._track = atoi(element->GetText());
				log("track = %d", note._track);
			}
			...
			else if (strcmp(element->Value(), "panel") == 0)
			{
				note._panel = atoi(element->GetText());
				log("panel = %d", note._panel);
			}
			else if (strcmp(element->Value(), "index") == 0)
			{
				note._index = atoi(element->GetText());
				log("index = %d", note._index);
			}
